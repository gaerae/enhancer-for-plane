...
3. **🔤 템플릿 변수** — 삽입 시 자동 치환: `{{date}}`(오늘, `YYYY-MM-DD`),
...
4. **📋 참조 복사** — 작업 항목 화면에서 번호 옆 버튼(또는 `Alt/⌥+C`)을 누르면 그 항목을
...
node tools/dom-harness.js   # 설정 화면과 팝업을 실제 브라우저에서 구동
```
...
`chrome` API를 대체한 스텁과 함께 헤드리스 Chrome으로 페이지를 띄워서 봅니다. 브라우저가
