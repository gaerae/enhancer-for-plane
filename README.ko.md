...
5. **🧘 집중 모드** — `Alt+Shift+F`(또는 팝업의 스위치)로 측면 패널을 숨겨 본문만 남깁니다.
   ...
   - 단축키는 페이지 안의 키 핸들러가 아니라 **브라우저 명령**입니다. 그래서 본문을 쓰는
     ...
     그 자체로 하나의 문자입니다. `chrome://extensions/shortcuts`에서 바꿀 수 있습니다.
...
- **본문에 집중**: `Alt+Shift+F`(또는 팝업의 집중 모드 스위치) → 속성 패널과 왼쪽 내비게이션이
  숨습니다. 같은 키로 되돌아오고, 그 탭에서만 유지됩니다.
