...
1. **⚡ 바로 열기** — 작업 항목 키를 입력하면 그 항목이 곧바로 열립니다. 주소창에 `issue`를 입력하고
   ...
   - **여러 낱말을 넣으면 검색합니다.** `issue login bug`처럼 입력하면 트래커의 검색 화면으로
     ...
     - Plane은 `https://내-plane/{워크스페이스}/search/?q={{q}}` 입니다. 검색어가 채워진 채로
       뜨는 **화면** 주소예요. 그 뒤의 `/api/…/search/?search=…` 가 아닙니다. 그건 앱이 보내는
       요청이라 그대로 열면 JSON이 나옵니다. API 주소처럼 보이면 설정에서 알려 줍니다.
