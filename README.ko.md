...
내려받을 때 적용되는 제한: 소스 **10개**, 소스당 템플릿 **200개**, 응답 **1 MB**,
...
- **동기화 저장소 한계(~8 KB).** 모든 설정(도메인 + 규칙 + 템플릿 + 변수 + 동기화
  소스)이 단일 `chrome.storage.sync` 항목에 저장되고, Chrome이 항목당 ~8 KB로
  제한합니다 — 일반적인 템플릿 기준 대략 20~30개. 설정 페이지에 실시간 용량 미터가 있어
  근접 시 경고하며, 저장 실패 시 템플릿을 줄이거나 JSON으로 백업(Export)하세요. 설정은
  로그인한 Chrome 기기 간에 동기화됩니다. **팀 템플릿은 이 한도를 쓰지 않습니다** —
  `chrome.storage.local`(5 MB, 이 기기 전용)에 저장되며, 공유 템플릿을 많이 두려면
  동기화가 답인 이유가 이것입니다.
