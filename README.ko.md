...
1. **⚡ 바로 열기** — 작업 항목 키를 입력하면 그 항목이 곧바로 열립니다. 주소창에 `issue`를 입력하고
   ...
   - **추가는 예시에서 시작합니다.** 추가 버튼을 누르면 이미 등록한 사이트마다 한 줄(호스트가
     채워진 채로)과 트래커별 한 줄(Plane, Jira, Linear, GitHub, GitLab)이 나옵니다. 고르면 두
     주소가 채워지고 아직 바꿔야 할 부분(`⟨워크스페이스⟩`, `⟨사이트⟩`)이 선택된 채로 커서가
     갑니다. 바로 타이핑하면 덮어써집니다. 모든 형태를 브라우저에서 직접 열어 확인했고, 검색
...
6. **🎯 비주얼 요소 피커** — 팝업의 **"🎯 Pick element → add rule"**을 누르고 화면의
   ...
   - **목록 순서는 "얼마나 정확한가"가 아니라 "다음 달에도 살아 있을까" 기준입니다.**
     ...
     `.max-w-40`)이 먼저 오고, 빌드가 만들어낸 것으로 보이면 아래로 내려가면서 **"곧 바뀔
     수 있음"** 배지가 붙습니다 — 해시 클래스(`sx-3nfvp2`), uuid(`editor-container-d833e58d-…`),
     CSS 모듈 이름, 카운터, React `useId` 값(`headlessui-menu-button-:r1b:`) 같은 것들요.
     내리기만 하고 숨기지는 않습니다. 남의 마크업에 대한 추측이니, 틀렸을 때 치를 값은
     순서지 선택지가 아니어야 합니다.
