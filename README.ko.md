...
이 익스텐션은 **자체 호스팅 Plane**을 위한 것입니다. **기본 활성 도메인이 없어**,
본인 인스턴스(예: `plane.your-company.com`)를 팝업의 **Enable on this site** 또는
설정의 활성 도메인에서 추가하기 전까지는 아무 동작도 하지 않습니다. 그 외 사이트에서는
완전히 비활성 상태로 남습니다.
...
- 콘텐츠 스크립트는 `<all_urls>` 매칭 — 자체 호스팅 Plane 호스트는 빌드 시점에 알 수
  없기 때문이며, 현재 호스트가 사용자가 활성화한 도메인과 일치하지 않으면 아무 동작도
  하지 않습니다.
...
- **자가 복구 주입.** 툴바가 늦게 마운트돼 버튼이 안 보여도 새로고침 없이 복구됩니다 —
  ...
  재주입. 콘텐츠 스크립트가 아예 로드되지 않았으면(설치 전부터 열린 탭 등) Re-scan이
  이를 알려 새로고침을 안내합니다.
...
1. `manifest.json`의 `"version"`을 올림(예: `1.0.0` → `1.0.1`).
2. 커밋 후 `main`에 푸시.
