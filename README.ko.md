...
3. **팀 템플릿 동기화** — JSON 파일 하나를 두면(사내 서버, Git 호스트, 아무 URL이나)
...
6. **활성 도메인** — 지정한 도메인에서만 동작. 와일드카드(`*.example.com`)와
   "모든 사이트에서 동작" 스위치 지원.
7. **백업(가져오기/내보내기)** — "Backup"에서 설정을 JSON으로 내보내고 되돌립니다.
   직접 설정한 것은 모두 담깁니다 — 도메인·규칙·템플릿·변수, 그리고 동기화 소스(URL,
...
이 파일을 손으로 쓸 필요는 없습니다. **설정 ▸ Team template sync ▸ "내 템플릿을 피드로
내보내기"** 를 누르면 지금 가진 템플릿이 아래 형식 그대로 저장됩니다. UI에서 만들고,
내보내고, 올리고, 주소를 공유하면 끝입니다. 내보낼 때 모음 이름(`name`)을 묻고
오늘 날짜를 `version`으로 찍습니다. 이것은 백업이 아니라 **배포용 파일**입니다. 내
템플릿만 담기며 도메인·규칙·변수 값·소스 URL은 들어가지 않습니다. 동기화로 *받은*
템플릿도 빠집니다. 그 템플릿은 배포한 쪽에서 관리합니다.
...
- **동기화 저장소 한계(~100 KB).** 설정은 `chrome.storage.sync`에 저장되며, Chrome이
  전체 ~100 KB, 항목당 ~8 KB로 제한합니다. 템플릿은 별도 항목(`peTpl.0`, `peTpl.1`, …)에
  나뉘어 담기므로, 한도는 항목 하나의 8 KB가 아니라 전체 100 KB입니다 — 일반적인 템플릿
  기준 수백 개. 여기서 두 가지가 따라옵니다. **템플릿 하나는 여전히 ~8 KB 항목 하나에
  들어가야 하며**, 이를 넘기는 저장은 어느 템플릿이 문제인지 알려주며 미리 거부됩니다.
  그리고 용량은 **바이트**로 셉니다. 한글이나 이모지가 많은 템플릿은 글자 수의 약 3배를
  차지합니다. 설정 페이지의 실시간 미터는 100 KB 기준으로 표시합니다. **팀 템플릿은 이
  한도를 쓰지 않습니다** — `chrome.storage.local`(10 MB, Chrome 113 이하는 5 MB, 이 기기
  전용)에 저장됩니다.
...
- `storage` — 사용자 설정(도메인·규칙·템플릿·변수·동기화 소스)을 저장하고, 내려받은
  팀 템플릿을 이 기기에 캐시합니다.
