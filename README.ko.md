...
4. **📐 스타일 규칙 — 범용 엔진**
   ...
   - 절단은 모듈만의 문제가 아닙니다. Plane은 **모듈과 사이클, 라벨, 브레드크럼(상단
...
6. **🎯 비주얼 요소 피커** — 팝업의 **"🎯 Pick element → add rule"**을 누르고 화면의
   ...
   - **목록 순서는 "얼마나 정확한가"가 아니라 "다음 달에도 살아 있을까" 기준입니다.**
     ...
     React `useId` 값(`headlessui-menu-button-:r1b:`) 같은 것들입니다. 내리기만 하고 숨기지는
...
9. **💾 백업(가져오기/내보내기)** — "Backup"에서 설정을 JSON으로 내보내고 되돌립니다.
   직접 설정한 것은 모두 담깁니다 — 도메인과 규칙, 템플릿, 변수, 복사 형식, 바로 열기 링크, 그리고
