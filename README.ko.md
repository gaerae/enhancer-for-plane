...
3. **📋 참조 복사** — 작업 항목 화면에서 번호 옆 버튼(또는 `Alt/⌥+C`)을 누르면 그 항목을
   ...
   - 항목 자신의 화면과 **목록에서 열리는 패널** 양쪽에서 동작합니다(헤더 구조가 같습니다).
     ...
     링크를 하나도 담고 있지 않아서, 거기서는 링크를 조립합니다 — 바로 열기에 등록해 둔
     링크가 있으면 그것으로, 없으면 `/{워크스페이스}/browse/{번호}` 형태로. 후자는 Plane의
     정식 짧은 링크이며 `/projects/{uuid}/issues/{uuid}`가 이 주소로 리다이렉트하는 것을
     확인했습니다.
   - **Jira와 Linear에서도 동작합니다** — 바로 열기에 링크를 등록해 둔 곳이면 어디든요.
     그 링크가 이미 사용자의 URL 문법(`https://linear.app/acme/issue/{{key}}`)을 적어 둔
     것이고, 문법은 양방향으로 읽힙니다. 주소창에서 번호로 항목을 여는 그 목록이, 지금 보고
     있는 페이지가 어떤 항목인지도 답해 줍니다. 제목은 페이지 제목에서 가져오는데 Plane과
     Linear 모두 `{번호} {제목}` 형태로 씁니다. 따로 설정할 것도, 디자인 변경에 밀릴 셀렉터도
     없습니다.
     - 복사되는 링크는 주소창이 아니라 **등록해 둔 템플릿이 말하는 정식 주소**입니다.
       Linear의 주소는 `/issue/GAE-1/get-familiar-with-linear`이지만 `/issue/GAE-1`이
       거기로 리다이렉트되는 쪽입니다. 즉 슬러그는 읽는 사람에게 필요 없으면서 붙여넣을
       때마다 따라오는 부분이고, 제목이 영어가 아니면 짧아지는 게 아니라 길어집니다.
       Plane에는 슬러그가 없어서 달라지는 것이 없습니다.
     - 그곳에서는 단축키가 유일한 방법입니다. 버튼은 작업 항목 헤더 옆에 붙어야 하는데
       헤더는 Plane의 구조이고, 다른 트래커의 마크업을 추측하는 것은 이 익스텐션이
       피해 온 결합입니다. `Alt/⌥+C`가 어디서 동작하는지는 설정의 **단축키** 탭에
       적혀 있습니다. 그 탭이 있는 이유가 이것입니다.
     - Plane에서는 헤더를 먼저 읽습니다. 취향이 아니라 필요입니다 — 목록에서 열리는
       패널은 주소창이 알지 못하는 항목이니까요. 둘 다 답할 수 있는 곳에서는 답이 같습니다.
